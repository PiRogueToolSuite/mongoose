...
def test_file_forwarder_basic_flow(output_dir):
    config = FileForwarderConfiguration(
...
def test_file_forwarder_multiple_topics(output_dir):
    config = FileForwarderConfiguration(
        output_dir=output_dir,
        topics=["network-alert", "network-flow"]
    )
    ...
    flow = NetworkFlow(
        flow_id=2, src_ip="3.3.3.3", src_port=443, dst_ip="4.4.4.4", dst_port=123,
        protocol="UDP", packets=5, bytes=500, start="2023-01-01T00:00:00", end="2023-01-01T00:00:01", age=1
    )
